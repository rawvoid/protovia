...
**Enum constants.** Java stays `Status.ACTIVE` and must be `UPPER_SNAKE_CASE` (`UNKNOWN`, not `Unknown`). The generated `.proto` prefixes every constant with the enum type name (`STATUS_ACTIVE`) so values stay unique under protobuf’s C++ scoping rules. `@ProtoEnum(name=…)` is the prefix source. `@ProtoReserved(names=…)` on an enum refers to those prefixed proto names.
